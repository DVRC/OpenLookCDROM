...
#define EXIT_SUNRASTERLOAD return( (Image *)0 )
#define EXIT_SUNREAD(zf) zclose(zf); return(-1)
...
static int sunread(zf, buf, len, enc)
     ZFILE        *zf;
     byte         *buf;
     unsigned int  len;
...
{ ZFILE          *zf;
  ...
  for (y= 0; y < image->height; y++) {
    if (sunread(zf, lineptr, linelen, enc) == -1)
      EXIT_SUNRASTERLOAD; 
    lineptr += linelen;
    if (fill)
      if (sunread(zf, &fillchar, fill, enc) == -1)
	EXIT_SUNRASTERLOAD; 
  }
  zclose(zf);
